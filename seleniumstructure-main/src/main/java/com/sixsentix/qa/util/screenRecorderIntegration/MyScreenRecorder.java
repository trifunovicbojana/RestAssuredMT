package com.sixsentix.qa.util.screenRecorderIntegration;
...
public class MyScreenRecorder extends ScreenRecorder {
    ...
    public String name;
    public static String nameOfRecording;

    //ToDo -> Sort wholce class, it's not readable

    public static void setNameOfRecording(String nameOfRecording) {
        MyScreenRecorder.nameOfRecording = nameOfRecording;
    }
    ...
    protected File createMovieFile(Format fileFormat) throws IOException {
        ...
        setNameOfRecording(name + "-" + dateFormat.format(new Date()) + "." + Registry.getInstance().getExtension(fileFormat));
    ...
    public static void removeRecording() {
        File f = new File("./recordings/" + nameOfRecording);
        if (f.exists()) {
            f.deleteOnExit();
        }
    }
    ...
    public static void startRecording(String methodName) throws Exception {
        ...
        screenRecorder = new MyScreenRecorder(gc, captureSize,
                new Format(FormatKeys.MediaTypeKey, MediaType.FILE, FormatKeys.MimeTypeKey, FormatKeys.MIME_QUICKTIME),
